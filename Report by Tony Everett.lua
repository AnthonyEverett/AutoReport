...
function main()
		...
		if sampIsLocalPlayerSpawned() and not spawned then
            for i, k in pairs(ips) do
                ...
                if ip == k then
					...
					sampAddChatMessage("» {9f7ec9}[Ëîâëÿ ðåïîðòà]{ffffff}: Ó âàñ óñòàíîâëåíà àêòóàëüíàÿ âåðñèÿ ñêðèïòà: {9f7ec9}"..thisScript().version.."{ffffff}!", -1)
                    sampAddChatMessage("» {9f7ec9}[Ëîâëÿ ðåïîðòà]{ffffff}: {FFFFFF}Ìåíþ íàñòðîéêè - {7eff39}/areport", -1)
                end
            end
				if thisScript().filename ~= "Report by Tony Everett.luac" then
    ...
    while true do wait(0)
		...
		sampAddChatMessage(u8'» {9f7ec9}[Ëîâëÿ ðåïîðòà] {ffffff}Àâòîìàòè÷åñêàÿ ëîâëÿ ðåïîðòà: {ff004d}'..(active and 'âêëþ÷åíà' or 'âûêëþ÷åíà'), -1)
          ...
          if f then
            ...
            if updateversion ~= thisScript().version then
              lua_thread.create(function(prefix)
                local dlstatus = require('moonloader').download_status
                sampAddChatMessage('» {9f7ec9}[Ëîâëÿ ðåïîðòà] {ffffff}: Îáíàðóæåíî îáíîâëåíèå. Ïûòàþñü îáíîâèòüñÿ c {9f7ec9}'..thisScript().version..'{ffffff} íà {9f7ec9}'..updateversion"", -1)
                wait(250)
                downloadUrlToFile(updatelink, thisScript().path,
                  function(id3, status1, p13, p23)
                    ...
                    elseif status1 == dlstatus.STATUS_ENDDOWNLOADDATA then
                      ...
                      sampAddChatMessage('» {9f7ec9}[Ëîâëÿ ðåïîðòà] {ffffff}: Îáíîâëåíèå çàâåðøåíî!', -1)
                      goupdatestatus = true
                      lua_thread.create(function() wait(500) thisScript():reload() end)
                    end
                    if status1 == dlstatus.STATUSEX_ENDDOWNLOAD then
                      if goupdatestatus == nil then
                        sampAddChatMessage('» {9f7ec9}[Ëîâëÿ ðåïîðòà] {ffffff}: Îáíîâëåíèå ïðîøëî íåóäà÷íî. Çàïóñêàþ óñòàðåâøóþ âåðñèþ...', -1)
                        update = false
                      end
                    end
                  end
...
function imgui.OnDrawFrame()
    if window.v then
        ...
        imgui.Begin(u8"Ëîâëÿ ðåïîðòà", window, imgui.WindowFlags.NoResize)
            imgui.Text(u8'Òåêóùàÿ êíîïêà ëîâëè') imgui.SameLine()
