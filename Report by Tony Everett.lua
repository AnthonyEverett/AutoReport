		...
		if sampIsLocalPlayerSpawned() and not spawned then
            for i, k in pairs(ips) do
                ...
                if ip == k then
					...
					if not sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))):find("Tony_Everett") then
					if not sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))):find("Tommy_Everett") then
					if not sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))):find("Jonathan_Everett") then
					sampAddChatMessage("» {9f7ec9}[Ловля репорта]{ffffff}: У вас нет доступа на использование данного скрипта! Получить доступ можно у разработчика", -1)
					sampAddChatMessage("» {9f7ec9}[Ловля репорта]{ffffff}: У вас нет доступа на использование данного скрипта! Получить доступ можно у разработчика", -1)
					sampAddChatMessage("» {9f7ec9}[Ловля репорта]{ffffff}: У вас нет доступа на использование данного скрипта! Получить доступ можно у разработчика", -1)
					sampAddChatMessage("» {9f7ec9}[Ловля репорта]{ffffff}: У вас нет доступа на использование данного скрипта! Получить доступ можно у разработчика", -1)
					sampAddChatMessage("» {9f7ec9}[Ловля репорта]{ffffff}: У вас нет доступа на использование данного скрипта! Получить доступ можно у разработчика", -1)
          ...
          if f then
            ...
            if updateversion ~= thisScript().version then
              lua_thread.create(function(prefix)
                local dlstatus = require('moonloader').download_status
                sampAddChatMessage('» {9f7ec9}[Ловля репорта] {ffffff}: Обнаружено обновление. Пытаюсь обновиться c {9f7ec9}'..thisScript().version..'{ffffff} на {9f7ec9}'..updateversion"", -1)
                wait(250)
                downloadUrlToFile(updatelink, thisScript().path,
                  function(id3, status1, p13, p23)
                    ...
                    elseif status1 == dlstatus.STATUS_ENDDOWNLOADDATA then
                      ...
                      sampAddChatMessage('» {9f7ec9}[Ловля репорта] {ffffff}: Обновление завершено!', -1)
                      goupdatestatus = true
                      lua_thread.create(function() wait(500) thisScript():reload() end)
                    end
                    if status1 == dlstatus.STATUSEX_ENDDOWNLOAD then
                      if goupdatestatus == nil then
                        sampAddChatMessage('» {9f7ec9}[Ловля репорта] {ffffff}: Обновление прошло неудачно. Запускаю устаревшую версию...', -1)
                        update = false
                      end
                    end
                  end
			  ...
			  sampAddChatMessage('» {9f7ec9}[Ловля репорта] {ffffff}: v'..thisScript().version..': Обновление не требуется.', -1)
            end
          end
        else
