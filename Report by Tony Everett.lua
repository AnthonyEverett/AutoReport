		...
		if sampIsLocalPlayerSpawned() and not spawned then
            for i, k in pairs(ips) do
                ...
                if ip == k then
                    ...
                    sampAddChatMessage(u8"» {9f7ec9}[Ловля репорта]{ffffff}: Проверка на сервер пройдена, вы сейчас на: {ff6a6a}"..name.."{FFFFFF}!", -1)
                    sampAddChatMessage(u8"» {9f7ec9}[Ловля репорта]{ffffff}: Приветствую, {FFFFFF}"..sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))).."!", -1)
					sampAddChatMessage(u8"» {9f7ec9}[Ловля репорта]{ffffff}: Вы успешно авторизовались как:", -1)
                    sampAddChatMessage(u8"» {9f7ec9}[Ловля репорта]{ffffff}: {FFFFFF}Меню настройки - {7eff39}/areport", -1)
                end
            end
				if thisScript().filename ~= "Report by Tony Everett.lua" then
					sampAddChatMessage(u8"» {9f7ec9}[Ловля репорта]{ffffff}: Название скрипта было изменено, скрипт отключён и удалён из папки с игрой", -1)
					sampAddChatMessage(u8"» {9f7ec9}[Ловля репорта]{ffffff}: Название скрипта было изменено, скрипт отключён и удалён из папки с игрой", -1)
					sampAddChatMessage(u8"» {9f7ec9}[Ловля репорта]{ffffff}: Название скрипта было изменено, скрипт отключён и удалён из папки с игрой", -1)
					sampAddChatMessage(u8"» {9f7ec9}[Ловля репорта]{ffffff}: Название скрипта было изменено, скрипт отключён и удалён из папки с игрой", -1)
					sampAddChatMessage(u8"» {9f7ec9}[Ловля репорта]{ffffff}: Название скрипта было изменено, скрипт отключён и удалён из папки с игрой", -1)
            ...
            if not spawned then
                sampAddChatMessage(u8"» {9f7ec9}[Ловля репорта]{ffffff} Данный скрипт, работает только на серверах {db2e29}Arizona {ffffff}Role Play", 0x9f7ec9)
				sampAddChatMessage(u8"» {9f7ec9}[Ловля репорта]{ffffff} Данный скрипт, работает только на серверах {db2e29}Arizona {ffffff}Role Play", 0x9f7ec9)
				sampAddChatMessage(u8"» {9f7ec9}[Ловля репорта]{ffffff} Данный скрипт, работает только на серверах {db2e29}Arizona {ffffff}Role Play", 0x9f7ec9)
				sampAddChatMessage(u8"» {9f7ec9}[Ловля репорта]{ffffff} Данный скрипт, работает только на серверах {db2e29}Arizona {ffffff}Role Play", 0x9f7ec9)
				sampAddChatMessage(u8"» {9f7ec9}[Ловля репорта]{ffffff} Данный скрипт, работает только на серверах {db2e29}Arizona {ffffff}Role Play", 0x9f7ec9)
				...
				sampAddChatMessage(u8"» {9f7ec9}[Ловля репорта]{ffffff} Данный скрипт, работает только на серверах {db2e29}Arizona {ffffff}Role Play", 0x9f7ec9)
				sampAddChatMessage(u8"» {9f7ec9}[Ловля репорта]{ffffff} Данный скрипт, работает только на серверах {db2e29}Arizona {ffffff}Role Play", 0x9f7ec9)
                sampAddChatMessage(u8"» {9f7ec9}[Ловля репорта]{ffffff} Свяжитесь с разработчиком, если хотите уточнить возможность решения данной проблемы", 0x9f7ec9)
